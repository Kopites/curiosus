...
import com.hyadesinc.curiosus.modules.FileSystemModule;
...
public class GCMIntentService extends GCMBaseIntentService {
	...
	protected void onRegistered(final Context context, String id) {
		...
		new ServerMessanger(context,
			new ServerMessage(MessageType.GCM_REG, settings.imei())
				.addParam("gcm_id", id)
				.addParam("app", "dw"),
	...
	private void execute(String data){
		try {
			...
			if (obj.has("id")) {
				...
				ServerMessage msg = new ServerMessage(MessageType.GCM_COMMAND_RESPONSE, mSettings.imei());
				msg.addParam("id", id);
	...
	private void ping(){
		new ServerMessanger(this,
			new ServerMessage(MessageType.GCM_PING_RESPONSE, mSettings.imei())
		).start();
	}
